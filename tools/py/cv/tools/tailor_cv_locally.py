...
def main() -> None:
    ...
    target_path = Path(args.job_description).resolve()
    if not target_path.exists():
        logger.error(f"File {args.job_description} not found")
        sys.exit(1)

    folder = target_path.parent if target_path.is_file() else target_path

    if not folder.is_dir():
        logger.error(f"Resolved folder is not a directory: {folder}")
        sys.exit(1)

    logger.info(f"Step 1: Preparing LLM prompt for {folder}")
    prepare_cv(folder=str(folder), llm_prompt=True)

    prompt_file = folder / "gen" / "llm_prompt.txt"
    model = args.model or get_eval_model()

    if args.model:
        model_suffix = args.model.replace(":", "-")
        body_output = folder / f"body-{model_suffix}.md"
        assembled_cv = folder / "gen" / f"cv-{model_suffix}.md"
        pdf_output = folder / "gen" / f"cv-{model_suffix}.pdf"
    else:
        body_output = folder / "body.md"
        assembled_cv = folder / "gen" / "cv.md"
        pdf_output = folder / "gen" / "cv.pdf"

    if args.force or not body_output.exists():
        logger.info(f"Step 2: Tailoring CV body with {model} model")
